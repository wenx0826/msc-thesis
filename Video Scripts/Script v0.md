...
We begin by selecting text passages relevant to the process. Multiple passages can be selected across the document. A selected passage can be adjusted directly. Its range can be refined by dragging the handles, and it can be recolored or removed using the toolbar. A color can also be chosen before making a selection. 

We now generate the model. 
An advanced mode is available if additional instructions are needed. 
Here, we proceed directly. 

Once the model is generated, it is presented as a draft for review. It can then be either saved or discarded.  Here, we save the model. The model is then saved, with a link to the text established.  
