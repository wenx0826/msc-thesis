function WFAdaptorManifestation(adaptor) {
  ...
  this.update_details = function (svgid) {
    ...
    if (self.adaptor.description.elements[$(node).attr("svg-subtype")]) {
      save["details_target"] = {
        svgid: svgid,
        model: self.adaptor.description,
      };
      var rng =
        self.adaptor.description.elements[$(node).attr("svg-subtype")].clone();
      if (
        save["endpoints_cache"][$(node).attr("endpoint")] &&
        save["endpoints_cache"][$(node).attr("endpoint")].schema
      ) {
        var schema =
          save["endpoints_cache"][$(node).attr("endpoint")].schema
            .documentElement;
      ...
      if (
        save["endpoints_list"][$(node).attr("endpoint")] &&
        (!save["endpoints_list"][$(node).attr("endpoint")].startsWith("http") ||
          save["endpoints_list"][$(node).attr("endpoint")].match(/^https?-/))
      ) {
        $(rng)
          .find(' > element[name="parameters"] > element[name="method"]')
          .remove();
      }
      save["details"] = new RelaxNGui(
        rng,
        tab,
        self.adaptor.description.context_eval,
        true
  ...
  function copyOrMove(menu, group, xml_node, mode) {
    ...
    if (
      nodes.length > 0 &&
      _.uniq(check1).length == _.intersection(check1, check2).length
    ) {
  ...
  function contextMenuHandling(svgid, e, child, sibling) {
    console.log("003 contextMenuHandling", svgid, e, child, sibling);
    console.log("004 save:", save);
    console.log("005 save state:", save["state"]);
    //{{{
    ...
    try {
      if (child) {
        ...
        if (
          self.elements[xml_node.get(0).tagName].permissible_children_expert
        ) {
          group = self.elements[
            xml_node.get(0).tagName
          ].permissible_children_expert(xml_node, "into");
          if (group.length > 0) {
      ...
      if (sibling) {
        group = self.elements[
          xml_node.parent().get(0).tagName
        ].permissible_children(xml_node, "after");
        if (group.length > 0) {
        ...
        if (
          self.elements[xml_node.parent().get(0).tagName]
            .permissible_children_expert
        ) {
          group = self.elements[
            xml_node.parent().get(0).tagName
          ].permissible_children_expert(xml_node, "after");
          if (group.length > 0) {
      ...
      if (
        xml_node.get(0).tagName != "description" &&
        !self.elements[xml_node.get(0).tagName].neverdelete
      ) {
        var icon =
          self.elements[xml_node.get(0).tagName].illustrator.svg.clone();
        ...
        if (nodes && nodes.length > 0) {
          ...
          menu["Delete"].push({
            ...
            function_call: function () {
              ...
              $(nodes).each(function (key, node) {
                var target = self.adaptor.description.get_node_by_svg_id(
                  $(node).attr("svg-id")
                );
                del_ui_pos(target);
      ...
      if (
        $("> code", xml_node).length > 0 &&
        xml_node.get(0).tagName == "call"
      ) {
      ...
      if (
        xml_node.get(0).tagName == "call" ||
        xml_node.get(0).tagName == "manipulate" ||
        ...
        if (vtarget.length > 0) {
          if (
            vtarget.parents("g.activities.passive, g.activities.active")
              .length > 0
          ) {
            menu["Position"] = [
              {
      ...
      console.log("004 Error in contextMenuHandling:", err);
    }
    console.log("004 Context Menu", e, menu);
  ...
  function positionHandling(svgid) {
    ...
    if (vtarget.length > 0) {
      if (
        vtarget.parents("g.activities.passive, g.activities.active").length > 0
      ) {
        del_ui_pos(xml_node);
      } else {
        add_ui_pos(xml_node);
  ...
  this.events.mousedown = function (svgid, e, child, sibling) {
    console.log("mousedown event", svgid, e, child, sibling);
    // {{{
    if (e.button == 0) {
      // left-click
    } else if (e.button == 1) {
  ...
  this.events.mouseover = function (svgid, e) {
    // {{{
    self.adaptor.illustrator.svg.container
      .find('.tile[element-id = "' + svgid + '"]')
      .css("display", "block");
    self.adaptor.illustrator.svg.container
      .find('[element-id = "' + svgid + '"]')
      .addClass("hover");
    self.adaptor.illustrator.svg.label_container
      .find('[element-id = "' + svgid + '"]')
      .addClass("hover");
    return false;
  }; // }}}
  ...
  this.elements.call = {
    ...
    illustrator: {
      ...
      label: function (node) {
        ...
        var threshold = $(
          "_timing_threshold",
          $(node).children("annotations")
        ).text();
        var adur = $("_timing_avg", $(node).children("annotations")).text();
        var lab = $("> label", $(node).children("parameters"))
          .text()
          .replace(/^['"]/, "")
          .replace(/['"]$/, "");
      ...
      resolve_symbol: function (node) {
        if (
          $("> annotations > _context_data_analysis > probes > probe", node)
            .length > 0
        ) {
  ...
  this.elements.manipulate = {
      ...
      label: function (node) {
        ...
        if (lab) {
          return [
            {
              column: "Label",
              value: lab.replace(/^['"]/, "").replace(/['"]$/, ""),
            },
            { column: "ID", value: $(node).attr("id") },
          ];
        } else {
    ...
    adaptor: {
      //{{{
      mousedown: function (node, e) {
        self.events.mousedown(node, e, false, true);
      },
      touchstart: function (node, e) {
        self.events.touchstart(node, e, false, true);
      },
      touchend: self.events.touchend,
      click: self.events.click,
      mouseover: self.events.mouseover,
  ...
  this.elements.terminate = {
    ...
    adaptor: {
      //{{{
      mousedown: function (node, e) {
        self.events.mousedown(node, e, false, false);
      },
      touchstart: function (node, e) {
        self.events.touchstart(node, e, false, false);
      },
      touchend: self.events.touchend,
      mouseover: self.events.mouseover,
      mouseout: self.events.mouseout,
    ...
    illustrator: {
      ...
      resolve_symbol: function (node) {
        if ($(node).children(":not(parallel_branch)").length > 0) {
          ...
          $(node).attr("cancel") == "last" &&
          $(node).attr("wait") == "-1"
        ) {
          return "parallel_simple";
        } else if (
          $(node).attr("cancel") == "first" &&
          $(node).attr("wait") == "-1"
        ) {
  ...
  this.elements.choose = {
        ...
        return [
          {
            column: "Label",
            value:
              $(node).attr("mode") == "exclusive" ? "exclusive" : "inclusive",
          },
        ];
      },
      endnodes: "aggregate",
    ...
    permissible_children: function (node, mode) {
      ...
      var func = null;
      if (mode == "into") {
        func = self.adaptor.description.insert_first_into;
      } else {
        func = self.adaptor.description.insert_after;
      }
      ...
      if (
        node.children("otherwise").length == 0 &&
        node.parents("parallel").length ==
          node.parents("parallel_branch").length &&
        node.parent("choose").length == 0
      )
        childs.push({
          label: "Otherwise",
          function_call: self.adaptor.description.insert_last_into,
      ...
      if (
        node.parents("parallel").length > node.parents("parallel_branch").length
      )
        childs.push({
          label: "Parallel Branch",
          function_call: func,
  ...
  this.elements.otherwise = {
    ...
    adaptor: {
      ...
      touchstart: function (node, e) {
        self.events.touchstart(node, e, true, false);
      },
      touchend: self.events.touchend,
      click: self.events.click,
      dblclick: self.events.dblclick,
  ...
  this.elements.alternative = {
    ...
    permissible_children: function (node, mode) {
      ...
      if (
        node.parents("parallel").length >
          node.parents("parallel_branch").length &&
        node.get(0).tagName == "alternative"
      ) {
        return [
          {
            label: "Parallel Branch",
  ...
  this.elements.loop = {
    ...
    permissible_children: function (node, mode) {
      ...
      if (
        node.parent("parallel").length > node.parent("parallel_branch").length
      ) {
        childs.push({
          label: "Parallel Branch",
          function_call: func,
  ...
  this.elements.parallel_branch = {
    ...
    illustrator: {
      ...
      col_shift: function (node) {
        ...
        if (
          $(node).parents("parallel").first().children(":not(parallel_branch)")
            .length > 0
        )
          return true;
        return false;
    ...
    permissible_children: function (node, mode) {
      ...
      if (
        node.parents("choose").length >
          node.parents("alternative, otherwise").length &&
        node.get(0).tagName == "parallel_branch"
      ) {
        return [
          {
            label: "Alternative",
  ...
  this.elements.critical = {
    ...
    permissible_children: function (node, mode) {
      //{{{
      console.log("critical permissible_children Node Mode", node, mode);
      var func = null;
      if (mode == "into") {
        func = self.adaptor.description.insert_first_into;
      } else {
