...
var myid = ([1e7] + -1e3 + -4e3 + -8e3 + -1e11).replace(/[018]/g, (c) =>
  (
    c ^
...
var graph_changed = new Event("graph:changed", {
  bubbles: true,
  cancelable: false,
});
...
var model_loaded = new Event("model:loaded", {
  bubbles: true,
  cancelable: false,
});
var save = {};
...
var sub_more =
  ...
  "events" +
  "=" +
  "calling,status,manipulating,failed,done" +
  "&" +
  "topic" +
  "=" +
  "activity" +
...
function cockpit() {
  //{{{
  $("button[name=base]").click(function () {
    create_instance($("input[name=base-url]").val(), null, false, false);
  });
  ...
  $("button[name=savebpmnfile]").click(function () {
    save_bpmnfile();
  });
  $("button[name=savesvgfile]").click(function () {
    save_svgfile();
  });
  $("button[name=state_start]").click(function () {
    $(this).parent().find("button").attr("disabled", "disabled");
    start_instance();
  });
  $("button[name=state_stop]").click(function () {
    $(this).parent().find("button").attr("disabled", "disabled");
    stop_instance();
  });
  $("button[name=state_abandon]").click(function () {
    aba_instance();
  });
  $("input[name=votecontinue]").click(check_subscription);
  $("input[name=testsetfile]").change(load_testsetfile_after);
  $("input[name=modelfile]").change(load_modelfile_after);
  $("#modifiers").on("change", "div.select select", function (e) {
    modifiers_update(e);
  });
  ...
  $.ajax({
    ...
    complete: function () {
      ...
      if (q.position) {
        graph_position = q.position == "false" ? "false" : "true";
      }
      if (q.highlight) {
      ...
      if (data["type"] == "event") {
        switch (data["topic"]) {
          ...
          case "attributes":
            ...
            if (
              typeof data.content.values.theme !== "undefined" &&
              save["graph_theme"] != data.content.values.theme
            ) {
  ...
  $.ajax({
    ...
    success: function (res) {
      ...
      history.replaceState(
        ...
        "?" +
          ("debug" in q ? "debug&" : "") +
          (graph_position ? "position=" + graph_position + "&" : "") +
          (graph_highlight ? "highlight=" + graph_highlight + "&" : "") +
        ...
        if (type == "endpoints") {
            ...
            .each(function (k, v) {
              ...
              $.ajax({
                ...
                success: () => {
                  tmp[v.tagName] = {};
                  $.when
                    .apply($, get_resource(rep, v.tagName, $(v).text(), tmp))
                    .then(function (x) {
              ...
              if (save["resources"]) {
                ...
                $.ajax({
                  url:
                    rep +
                    "endpoints/" +
                    encodeURIComponent(encodeURIComponent($(v).text())),
                  success: () => {
                    tmp[v.tagName] = {};
                    $.when
                      .apply(
          ...
          if ($("#modifiers").length > 0) {
            if ($("#modifiers > div").length == 0) {
              modifiers_display().then(function () {
                modifiers_select();
              });
            } else {
              modifiers_select();
...
function adaptor_init(url, theme, dslx) {
  ...
  if (save["graph_theme"] != theme) {
    ...
    save["graph_adaptor"] = new WfAdaptor(
      $("body").data("theme-base") + "/" + theme + "/theme.js",
        ...
        graphrealization.draw_labels = (max, labels, dimensions, striped) => {
          ...
          if (graph_highlight) {
            graph_highlight_tasks.forEach((ele) => {
              ...
              styletext =
                'svg rect[element-row="' +
                er +
                '"], div.graphlast[element-row="' +
                er +
                '"] {  fill: #' +
      ...
      if (
        ...
        (save["graph"] &&
          save["graph"].serializePrettyXML() !=
            $(dslx.documentElement).serializePrettyXML())
      ) {
        $.ajax({
          type: "GET",
...
function monitor_instance_running(content, event) {
  ...
  if (event == "calling") {
    ...
    save["activity_red_states"][content["activity-uuid"]] = true;
    setTimeout(() => {
      delete save["activity_red_states"][content["activity-uuid"]];
    }, 1);
  }
} // }}}
...
function monitor_instance_state_change(notification) {
  ...
  if (
    ...
    save["states"]["stopping"] - save["states"]["stopped"] < 10
  )
    notification = "stopped";
  if (notification == "stopping" && save["state"] == "stopped") return;
...
function monitor_instance_vote_add(parts) {
  ...
  if ($("input[name=votecontinue]").is(":checked"))
    astr +=
      "<button id='vote_to_continue-" +
      parts.activity +
      "-" +
      parts.key +
      '\' onclick=\'$(this).attr("disabled","disabled");monitor_instance_vote_remove("' +
      parts.activity +
      '","' +
      parts.key +
      '","true");\'>' +
...
function load_testsetfile_after() {
  ...
  reader.onabort = function () {
    console.log("abort reading file");
    loading = false;
  };
  ...
  $("testset > subscriptions > *", testset).each(async function () {
    ...
    if (typeof vals[suburl] == "undefined") {
      if ($("*", han).length > 0) {
        let inp = [];
        if (sid) {
          inp.push("id=" + encodeURIComponent(sid));
        }
...
function format_visual_add(what, cls, sum = true) {
  //{{{
  if (node_state[what] == undefined) node_state[what] = {};
  if (node_state[what][cls] == undefined) node_state[what][cls] = 0;
...
function format_visual_remove(what, cls, sum = true) {
  ...
  if (node_state[what][cls] == undefined) node_state[what][cls] = 0;
  node_state[what][cls] -= 1;
  if (node_state[what][cls] < 0) node_state[what][cls] = 0;
...
function scroll_into_view(what) {
  ...
  if ($('g[element-id="' + what + '"]').length > 0) {
    ...
    if (tcontainer.scrollTop + tcontainer.offsetHeight - 40 < telement) {
      tcontainer.scroll({
        top: telement - tcontainer.offsetHeight + 40,
        behavior: "smooth",
      });
    }
  }
...
function format_visual_set(what) {
  ...
  if (node_state[what] != undefined) {
    ...
    if (node_state[what]["passive"] == undefined)
      node_state[what]["passive"] = 0;
    ...
    $.each(
      ["#activity-" + what, 'g[element-id="' + what + '"] > g'],
      function (i, t) {
        $(t).each(function (a, b) {
...
function format_visual_clear() {
  //{{{
  node_state = {};
  $(".super .active").each(function (a, b) {
    b.setAttribute("class", "active");
  });
  $(".super .passive").each(function (a, b) {
    b.setAttribute("class", "passive");
  });
  $(".super .vote").each(function (a, b) {
    b.setAttribute("class", "vote");
  });
  $(".super .colon").each(function (a, b) {
    b.setAttribute("class", "colon");
  });
...
function append_to_log(what, type, message) {
  ...
  if (type == "description/change") {
    $("#dat_log").prepend(
      "<tr><td class='fixed'><a title=\"" +
        d.strftime("[%d/%b/%Y %H:%M:%S]") +
        "\">D</a></td><td class='fixed'>&#160;-&#160;</td><td class='fixed'><a title=\"" +
        what +
