...
var RelaxNGui = function (rng, target, ceval, ignore = false) {
  if (!(rng instanceof XMLDocument)) {
    // rng has to be XMLDocument //{{{
  ...
  var rec_lenextract = function (elements, lencount = 0) {
    ...
    $.each(elements, function (k, v) {
      ...
      if (
        ...
        tag.namespaceURI == "http://relaxng.org/ns/structure/1.0"
      ) {
        $(tag)
          .children("attribute")
          .each(function (l, w) {
  ...
  var lenextract = function (tag, lencount) {
    ...
    $.each(tag.attributes, function (k, v) {
      ...
      if (
        v.localName == "labellength" &&
        v.namespaceURI == "http://rngui.org"
      ) {
        lencount = parseInt(v.nodeValue);
      }
    });
    return lencount;
  }; //}}}
  ...
  var labextract = function (type, tag) {
    ...
    $.each(tag.attributes, function (k, v) {
      if (v.localName == "label" && v.namespaceURI == "http://rngui.org") {
        ret["label"] = v.nodeValue;
      }
      if (v.localName == "readonly" && v.namespaceURI == "http://rngui.org") {
        ret["readonly"] = v.nodeValue == "true" ? true : false;
      }
      if (v.localName == "wrap" && v.namespaceURI == "http://rngui.org") {
        ret["wrap"] = v.nodeValue == "true" ? true : false;
      }
    });
    $.each(tag.children, function (k, v) {
      if (
        v.localName == "param" &&
        v.namespaceURI == "http://relaxng.org/ns/structure/1.0"
      ) {
  ...
  var addelements = function (target, container, first) {
    ...
    template
      .find(".relaxngui_template [data-relaxngui-template]")
      .each(function (j, t) {
        $(t).attr("data-relaxngui-template", true);
      });
    template.addClass("relaxngui_dyn");
    template.on("click", "> [data-type=del]", function (ev) {
      delelements($(ev.target));
    });
  ...
  var recshow_header = function (tag, ret, level) {
    ...
    var fold;
    $.each(tag.attributes, function (k, v) {
      if (v.localName == "header" && v.namespaceURI == "http://rngui.org") {
        header = v.nodeValue;
      }
      if (v.localName == "fold" && v.namespaceURI == "http://rngui.org") {
        fold = v.nodeValue;
      }
    });
    if (header) {
      if (fold != undefined) {
        var xxx = $(
          "<div data-relaxngui-level='" +
            level +
            "' class='relaxngui_header fold'>" +
            header +
            "</div>"
        );
  ...
  var recshow_data = function (
    ...
    var os = optional
      ? ' onkeyup=\'var sl = $(this).siblings("label"); if ($(this).get_val() == "") { if (!sl.hasClass("optional")) { sl.addClass("optional") } } else { sl.removeClass("optional") }\' data-optional=\'true\''
      : " data-optional='false'";
    if (second.readonly)
      node.append(
        $(
          "<input      data-relaxngui-visible='true'                                                                  " +
            (defaul && typeof defaul == "string"
              ? 'value="' + defaul + '"'
              : "") +
            " data-relaxngui-template='" +
            template +
            "' data-relaxngui-parent='" +
            path +
            "' data-relaxngui-path='" +
            tpath +
            "' class='relaxngui_cell' type='text'                                                                              title='" +
      ...
      if (second.type == "string") {
        ...
        node.append(
          $(
            "<input    data-relaxngui-visible='true'                                                                  " +
              ...
              "' data-relaxngui-path='" +
              tpath +
              "' class='relaxngui_cell' type='text'     pattern='([\\+\\-]?[0-9]+)|(^!.*)'                                       title='" +
              (second.label != "" ? second.label : "integer number") +
              "'              id='" +
              id +
              "' placeholder='" +
              second.label +
              "'" +
              (second.min != undefined ? " min='" + second.min + "'" : "") +
              (second.max != undefined ? " max='" + second.max + "'" : "") +
              os +
              "></input>"
          )
        );
        ...
        node.append(
          $(
            "<input    data-relaxngui-visible='true'                                                                  " +
              ...
              "' data-relaxngui-path='" +
              tpath +
              "' class='relaxngui_cell' type='text'     pattern='([\\+]?[1-9]+[0-9]*)|(^!.*)'                                    title='" +
              (second.label != "" ? second.label : "positive integer number") +
              "'     id='" +
              id +
              "' placeholder='" +
              second.label +
        ...
        node.append(
          $(
            "<input    data-relaxngui-visible='true'                                                                  " +
              ...
              "' data-relaxngui-path='" +
              tpath +
              "' class='relaxngui_cell' type='text'     pattern='([0-9]{4}-(0[1-9]|1[012])-(0[1-9]|1[0-9]|2[0-9]|3[01]))|(^!.*)' title='" +
              (second.label != "" ? second.label : "date") +
              "'                        id='" +
              id +
              "' placeholder='" +
              second.label +
        ...
        node.append(
          $(
            "<input    data-relaxngui-visible='true'                                                                  " +
              ...
              "' data-relaxngui-path='" +
              tpath +
              "' class='relaxngui_cell' type='text'     pattern='([\\+\\-]?[0-9]+([.][0-9]*)?|[.][0-9]+)|(^!.*)'                 title='" +
              (second.label != "" ? second.label : "decimal number") +
              "'              id='" +
              id +
              "' placeholder='" +
              second.label +
        ...
        node.append(
          $(
            "<input    data-relaxngui-visible='true'                                                                  " +
              ...
              "' data-relaxngui-path='" +
              tpath +
              "' class='relaxngui_cell' type='checkbox'                                                                          title='" +
              (second.label != "" ? second.label : "boolean") +
              "'                     id='" +
              id +
              "' placeholder='" +
              second.label +
        ...
        tnode.append(
          "<span class='round none'><input data-relaxngui-visible='true' data-relaxngui-template='" +
            template +
            "' data-relaxngui-parent='" +
            path +
            "' data-relaxngui-path='" +
        ...
        $.each(datalist, function (didx, dname) {
          tnode.append(
            "<span class='round'><input data-relaxngui-visible='true' data-relaxngui-template='" +
              template +
              "' data-relaxngui-parent='" +
              path +
              "' data-relaxngui-path='" +
        ...
        node.append(
          $(
            "<ol      data-relaxngui-visible='true' contenteditable='true' data-relaxngui-wrap='" +
              second.wrap +
              "' " +
              (defaul && typeof defaul == "string"
                ? 'value="' + defaul + '"'
        ...
        var tnode = $(
          "<select   data-relaxngui-visible='true' " +
            (defaul && typeof defaul == "string"
              ? 'value="' + defaul + '"'
              : "") +
            "                                                                  data-relaxngui-template='" +
        ...
        $.each(datalist, function (didx, dname) {
          if (dname[1] == defaul)
            ...
            tnode.append(
              '<option value="' + dname[1] + '">' + dname[0] + "</value>"
            );
        });
        node.append(tnode);
      } else {
        node.append(
          $(
            "<input    data-relaxngui-visible='true' value=''                                                                                                                                        data-relaxngui-template='" +
              template +
              "' data-relaxngui-parent='" +
              path +
              "' data-relaxngui-path='" +
  ...
  var recshow_single = function (tag, ret, template, path, lencount, optional) {
    ...
    $.each(tag.attributes, function (k, v) {
      if (v.localName == "name") {
        first.name = v.nodeValue.replace(/:/, "\\:");
      }
      if (v.localName == "label" && v.namespaceURI == "http://rngui.org") {
        first.label = v.nodeValue;
      }
      if (v.localName == "labeltype" && v.namespaceURI == "http://rngui.org") {
        first.labeltype = v.nodeValue;
      }
      if (v.localName == "default" && v.namespaceURI == "http://rngui.org") {
        first.default = v.nodeValue;
      }
      if (v.localName == "visible" && v.namespaceURI == "http://rngui.org") {
        first.visible = v.nodeValue == "false" ? false : true;
      }
      if (v.localName == "onchange" && v.namespaceURI == "http://rngui.org") {
        first.onchange = v.nodeValue;
      }
      if (v.localName == "oninit" && v.namespaceURI == "http://rngui.org") {
        first.oninit = v.nodeValue;
      }
      if (v.localName == "hint" && v.namespaceURI == "http://rngui.org") {
        first.hint = v.nodeValue;
      }
    });

    $.each($(tag).children("data[type=string]"), function (k, v) {
      second = labextract("string", v);
    });
    $.each($(tag).children("data[type=integer]"), function (k, v) {
      second = labextract("integer", v);
    });
    $.each($(tag).children("data[type=nonNegativeInteger]"), function (k, v) {
      second = labextract("nonNegativeInteger", v);
    });
    $.each($(tag).children("data[type=positiveInteger]"), function (k, v) {
      second = labextract("positiveInteger", v);
    });
    $.each($(tag).children("data[type=float]"), function (k, v) {
      second = labextract("float", v);
    });
    $.each($(tag).children("data[type=date]"), function (k, v) {
      second = labextract("date", v);
    });
    $.each($(tag).children("data[type=boolean]"), function (k, v) {
      second = labextract("bool", v);
    });
    $.each($(tag).children("text"), function (k, v) {
      second = labextract("text", v);
    });
    $.each(
      $(tag).children("attribute[name=rngui-nonfunctional]"),
      function (k, v) {
        first.functional = false;
      }
    );
    $.each(
      $(tag).children("choice, data[type=string] > choice"),
      function (k, v) {
        second = labextract("datalist", v);
        if (
          v.hasAttributeNS("http://rngui.org", "href") &&
          v.hasAttributeNS("http://rngui.org", "extract")
        ) {
          let href_url = v.getAttributeNS("http://rngui.org", "href");
          if (
            href_url &&
            typeof href_url == "string" &&
            href_url.match(/^javascript:/)
          ) {
        ...
        $.each($(v).children("value"), function (k, v) {
          datalist.push([
            v.hasAttributeNS("http://rngui.org", "label")
              ? v.getAttributeNS("http://rngui.org", "label")
              : v.textContent,
            v.textContent,
          ]);
        });
    ...
    if (first.name && first.label) {
      node.append(
        $(
          "<label class='relaxngui_cell" +
            (optional && first.default == null && first.visible
              ? " optional"
              : "") +
            "' style='min-width: " +
            lencount +
            ...
            "'>" +
            first.label +
            "</label><span class='relaxngui_cell'>⇒</span>"
        )
      );
    } else if (first.name) {
      ...
      if (first.labeltype == "xml") {
        node.append(
          $(
            "<input data-relaxngui-visible='true' data-relaxngui-template='" +
              template +
              "' data-relaxngui-parent='" +
              path +
              "' data-relaxngui-path='" +
              path +
              " > *[data-name]' class='relaxngui_cell' type='text' pattern='^[a-zA-Z_][a-zA-Z0-9_\\-]*$' placeholder='" +
              first.label +
              "'></input><span class='relaxngui_cell'>⇒</span>"
          )
        );
      } else {
        node.append(
          $(
            "<input data-relaxngui-visible='true' data-relaxngui-template='" +
              ...
              "' data-relaxngui-path='" +
              path +
              " > *[data-name]' class='relaxngui_cell' type='text' pattern='^[a-z_][a-zA-Z0-9_]*$'       placeholder='" +
              first.label +
              "'></input><span class='relaxngui_cell'>⇒</span>"
          )
        );
    ...
    if (first.label) {
      ...
      if (tag.localName != "element") {
        // its an attribute, simulate its empty-ness
  ...
  var recshow = function (elements, template, path, attr) {
    ...
    $.each(elements, function (k, v) {
      ...
      if (
        ...
        if (template) {
          var yyy = $(
            '<div data-relaxngui-level="' +
              attr.level +
              '" class="relaxngui_table ' +
              attr.mode +
              '" data-relaxngui-template="true" data-relaxngui-parent="' +
              path +
              '" data-relaxngui-path="' +
              (path == ""
                ? " > " +
                  (typeof elements.attr("name") === "undefined"
                    ? "*"
                    : elements.attr("name"))
                : path +
                  " > " +
                  (typeof $(tag).attr("name") === "undefined"
                    ? "*"
                    : $(tag).attr("name"))) +
              '[data-main]">'
          );
          xxx = $(
            '<div class="relaxngui_template"><div data-type="del">✖</div><div data-type="mov"><div>⬍</div></div></div>'
          );
          xxx.append(yyy);
          ret.append(xxx);
          xxx = yyy;
          ...
          if ($(tag).children().length == 0) {
            ...
            xxx = $(
              '<div data-relaxngui-level="' +
                attr.level +
                '" class="relaxngui_hidden relaxngui_table ' +
                attr.mode +
                '" data-relaxngui-template="false" data-relaxngui-parent="' +
                path +
                '" data-relaxngui-path="' +
                (path == ""
                  ? " > " + elements.attr("name")
        ...
        var rcount = 0;
        $(tag)
          .children("attribute:not([name=rngui-nonfunctional])")
          .each(function (l, w) {
            var ttag = $(w)[0];
            rcount += recshow_single(
              ttag,
              xxx,
              template,
              path + " > " + $(tag).attr("name"),
        ...
        if (
          (sub = recshow(
            $(tag).children("element, zeroOrMore, optional"),
            false,
            path + " > " + $(tag).attr("name"),
            {
              ignore: attr.ignore,
              mode: attr.mode == "even" && rcount % 2 == 0 ? "odd" : "even",
              level: attr.level + 1,
            }
          ))
        ) {
        ...
        but.on("click", function (ev) {
          addelements($(ev.target), $(ev.target).parent(), false);
        });

        ret.append(
          recshow($(tag).children(), true, path, {
            ignore: attr.ignore,
            mode: attr.mode,
            level: attr.level,
          })
        ...
        ret.append(
          recshow($(tag).children("element, zeroOrMore"), false, path, {
            ignore: attr.ignore,
            mode: attr.mode == "even" ? "odd" : "even",
            optional: true,
            level: attr.level + 1,
            lencount: lencount,
          })
      ...
      $(tag)
        ...
        .each((_, ele) => {
          if (
            ele.localName == "script" &&
            ele.namespaceURI == "http://rngui.org"
          ) {
            let scr = $("<script/>");
            scr.text($(ele).text());
            ret.append(scr);
  ...
  this.save_raw = function () {
    ...
    var curr;
    var tar = target.find(
      "[data-relaxngui-path][data-relaxngui-visible=true]:not([data-relaxngui-template=true])"
    );
        ...
        if (ma.match(/\*\[data-main\]/)) {
          ...
          if ($(tar[i]).get_val()) {
            ...
            if (
              ...
              var nn = $(
                $XR("<" + $(tar[i]).get_val() + ' xmlns="' + parent_ns + '"/>')
                  .documentElement
              ).text($(tar[i + 1]).get_val());
            } else {
              var nn = $(
                $XR("<" + $(tar[i]).get_val() + "/>").documentElement
              ).text($(tar[i + 1]).get_val());
            }
          ...
          att[1] = att[1].replace(/\\/, "");
          $(parent_path + ":last-child", xml)
            .last()
            .attr(att[1], $(tar[i]).get_val());
        } else {
          if (
            $(tar[i]).attr("data-optional") == "true" &&
            $(tar[i]).get_val() == ""
          ) {
            $(path + ":last-child", xml)
              .last()
    ...
    console.log("RelaxnGUI: XML content:", xml);
    return xml;
  }; //}}}
  this.save = function () {
    //{{{
    console.log("RelaxnGUI: Saving XML content.????");
  ...
  this.content = function (data) {
    ...
    if (data) {
      ...
      if (x != y) {
        ...
        target.find("[data-relaxngui-path]").each(function (k, pa) {
          ...
          if (!path.match(/data-\w+\]$/)) {
            if (
              $(pa).attr("data-relaxngui-template") == "true" &&
              path.match(/\*$/)
            ) {
              $(data)
                .find(path)
                  ...
                  $(
                    target
                      .find(
                        '[data-relaxngui-path="' +
                          path +
                          '[data-name]"][data-relaxngui-template="false"]'
                      )
                      .get(index)
                  ).set_val(ele.localName);
                  $(
                    target
                      .find(
                        '[data-relaxngui-path="' +
                          path +
                          '"][data-relaxngui-template="false"]'
                      )
                      .get(index)
                  ).set_val($(ele).text());
                });
            ...
            if ($(pa).attr("data-relaxngui-template") == "true") {
              var but = target
                .find(
                  '.relaxngui_table[data-relaxngui-template="false"] > .relaxngui_template > [data-relaxngui-path="' +
                    path +
                    '"][data-relaxngui-template="true"]'
                )
                .parent()
                .parent()
                .find("> button");
      ...
      target.find(".relaxngui_table.closed_conditional").each(function (k, e) {
        $(e)
          .find("input:not([readonly]), [contenteditable]:not([readonly])")
          .each(function (l, f) {
            if ($(f).get_val() != "") {
      ...
      $("[data-relaxngui-oninit]", target).each((i, ele) => {
        ...
        relaxngui_init_target = $(ele)
          .attr("data-relaxngui-path")
          .replace(/\[data-main\]/, "");
        eval(ev);
      });
    }
  ...
  (function ($) {
    ...
    $.fn.movelements = function (item) {
      ...
      $(document).on("mousemove", function (e) {
        if (!item.hasClass("moveable")) return;
        ...
        if (
          prev.length > 0 &&
          e.originalEvent.clientY < prev.offset().top + prev.outerHeight()
        ) {
          $(prev).before($(item));
          pos = e.originalEvent.clientY;
        } else if (
          next.length > 0 &&
          e.originalEvent.clientY > next.offset().top
        ) {
          $(next).after($(item));
          pos = e.originalEvent.clientY;
        }
  ...
  if ($("#relaxngui_ruler").length == 0) {
    $("body").prepend(
      $(
        '<span id="relaxngui_ruler" style="position: absolute; visibility: hidden; white-space: nowrap;"></span>'
      )
    );
  }

  target.append(
    recshow($(rng.documentElement), false, "", {
      ignore: ignore,
      mode: "even",
      level: 0,
    })
  ...
  this.relaxngui_visible = function (what, ...args) {
    let curval = $(
      '[data-relaxngui-path="' + relaxngui_init_target + '"]',
      target
    ).get_val();
    if (what == curval) {
      args.forEach((arg, i) => {
  ...
  target.on(
    ...
    function (ev) {
      ...
      let pp = $(ev.currentTarget).attr("data-relaxngui-path") + "[data-main]";
      let current_element = $(ev.currentTarget);
      let current_element_main = $(
        '[data-relaxngui-path="' + pp + '"]',
        target
      );
      if (current_element_main.is("[data-relaxngui-onchange]")) {
        eval(current_element_main.attr("data-relaxngui-onchange"));
