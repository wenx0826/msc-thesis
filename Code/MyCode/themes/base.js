function WFAdaptorManifestationBase(adaptor) {
  ...
  this.get_details = function (svgid) {
    ...
    console.log("Node for svgid:", node);
  ...
  this.get_linked_model = function (svgid) {
    var node = self.adaptor.description.get_node_by_svg_id(svgid).get(0);
    var model = (model = $("> parameters > arguments > model", node).text());
    console.log("Node for svgid:", model);
    return model;
  };
  ...
  var contextMenuHandling_clean_icon = (this.contextMenuHandling_clean_icon =
    function (icon) {
      //{{{
      icon = icon.clone();
  ...
  var contextMenuHandling = (this.contextMenuHandling = function (
    svgid,
    e,
    child,
    sibling
    ...
    if ($("> code", xml_node).length > 0 && xml_node.get(0).tagName == "call") {
      ...
      icon
        ...
        .css("fill", "var(--wfadaptor-important");
      menu["Delete"].push({
    ...
    if (
      ...
      if (vtarget.length > 0) {
        if (
          vtarget.parents("g.activities.passive, g.activities.active").length >
          0
        ) {
          menu["Position"] = [
            {
    ...
    illustrator: {
      ...
      label: function (node) {
        var ret = [
          {
            column: "Label",
            value: $("> label", $(node).children("parameters"))
              .text()
              .replace(/^['"]/, "")
              .replace(/['"]$/, ""),
          },
      ...
      resolve_symbol: function (node) {
        if (
          ...
          if ($("> code", node).length > 0) {
            if (
              $("> code > signal", node).length > 0 &&
              $("> code > signal", node).text() == "on"
            ) {
  ...
  this.elements.choose = {
    ...
    illustrator: {
      //{{{
      label: function (node) {
        return [{ column: "Label", value: $(node).attr("label") }];
      },
      endnodes: "aggregate",
      closeblock: false,
  ...
  this.elements.start = this.elements.description = {
    ...
    illustrator: {
      ...
      resolve_symbol: function (node) {
        ...
        var regassi =
          /data\.([a-z_][a-zA-Z0-9_]*)\s*(=[^=]|\+\=|\-\=|\*\=|\/\=|<<|>>|\|\|=)/g; // we do not have to check for &gt;/&lt; version of stuff as only conditions are in attributes, and conditions can not contain assignments
        ...
        $(
          ...
          if (n.hasAttribute("condition")) {
            ...
            if (n.nodeName == "manipulate") {
              // css selector can not directly access manipulate
              item += n.textContent + "\n";
            }
