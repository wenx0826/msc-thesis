...
function WfAdaptor(theme_base, doit) {
  // Controller {{{
  console.info(
    "001-WfAdaptor: Initializing with theme '" + theme_base + "'",
    "doit:",
    doit
  );
  // public variables {{{
  ...
  this.draw_labels = function (max, labels, height_shift, striped) {
    // public {{{
  }; // }}}
    ...
    for (element in manifestation.elements) {
      ...
      if (manifestation.elements[element].description) {
        ...
        if ($.isArray(manifestation.elements[element].description)) {
          for (const [ind, val] of Object.entries(
            manifestation.elements[element].description
          )) {
            deferreds.push(
              $.ajax({
...
function WfIllustrator(wf_adaptor) {
  console.info("002-WfIllustrator: Initializing");
  // View  {{{
  // Variable {{{
  // public
  this.height = 40;
  ...
  this.set_svg = function (graph) {
    ...
    self.svg.container.attr(
      "height",
      graph.max.row * self.height + self.height_shift
    );
  ...
  var draw_stripe = (this.draw.draw_stripe = function (row, maxcol) {
    ...
    var g = $X(
      '<rect element-row="' +
        ...
        '" width="' +
        (self.width * maxcol + self.width - self.width_shift) +
        '" height="' +
        self.height +
  ...
  var draw_label = (this.draw.draw_label = function (
    ...
    // {{{
    var g = $X(
      '<text class="label" transform="translate(' +
        String(col * self.width - self.width_shift) +
        "," +
        String(row * self.height + 20 - (self.height - self.height_shift)) +
    ...
    spli.each(function (k, v) {
      var tspan = $X(
        '<tspan x="0" dy="' +
          (spli.length > 1 ? "-7" : "0") +
          '" xmlns="http://www.w3.org/2000/svg"></tspan>'
      );
      if (k == 0) {
        tspan.text(v);
      } else {
    ...
    if (addition) {
      var g = $X(
        '<g class="element" element-type="' +
          ...
          String(col * self.width - self.width_shift) +
          "," +
          String(row * self.height - (self.height - self.height_shift)) +
          ')"></g>' +
          "</g>"
      );
    } else {
    ...
    group.prepend(
      $X(
        '<rect element-id="' +
          id +
          '" x="' +
          (p1.col - 0.5) * self.width +
          '" ' +
          'y="' +
          (p1.row - 0.8) * self.height +
          '" ' +
          'width="' +
          (p2.col + 1.0 - p1.col) * self.width +
          '" ' +
          'height="' +
          (p2.row + 1.0 - p1.row) * self.height +
          '" ' +
    ...
    group.prepend(
      $X(
        '<rect element-id="' +
          id +
          '" x="' +
          ((p1.col - 1) * self.width + 1.3 * self.width_shift) +
          '" ' +
          'y="' +
          ((p1.row - 1) * self.height + self.height_shift / 2) +
          '" ' +
          'width="' +
          (p2.col + 1 - p1.col) * self.width +
          '" ' +
          'height="' +
          (p2.row + 1 - p1.row) * self.height +
          '" ' +
  ...
  var draw_connection = (this.draw.draw_connection = function (
    ...
    if (arrow)
      line = $X(
        '<path xmlns="http://www.w3.org/2000/svg" class="ourline" marker-end="url(#arrow)"/>'
      );
    else
    ...
    if (end["row"] - start["row"] == 0 || end["col"] - start["col"] == 0) {
      ...
      line.attr(
        "d",
        "M " +
          String(start["col"] * self.width) +
          "," +
          String(start["row"] * self.height - 15) +
          " " +
          String(end["col"] * self.width) +
          "," +
      ...
      if (end["col"] - start["col"] > 0) {
        ...
        if (self.compact) {
          line.attr(
            ...
            "M " +
              ...
              String(start["row"] * self.height - 15) +
              " " +
              String(start["col"] * self.width + 14) +
              "," +
              String((end["row"] - 1) * self.height) +
              " " + // first turn of hotizontal-line going away from node
              String(end["col"] * self.width) +
              "," +
              String((end["row"] - 1) * self.height) +
              " " +
        ...
        line.attr(
          ...
          "M " +
            ...
            String(start["col"] * self.width) +
            "," +
            String(end["row"] * self.height - 35) +
            " " +
            String(end["col"] * self.width + 14) +
            "," +
            String(end["row"] * self.height - 35) +
      ...
      if (num_lines > 1) {
        ...
        line.attr(
          ...
          "M " +
            ...
            String(start["col"] * self.width) +
            "," +
            String((max_line - 1) * self.height + 5) +
            " " +
            String(end["col"] * self.width + 20) +
            "," +
            String((max_line - 1) * self.height + 5) +
            " " +
            String(end["col"] * self.width + 20) +
            "," +
            String(end["row"] * self.height + 25) +
            " " +
            String(end["col"] * self.width) +
            "," +
            String(end["row"] * self.height - 15)
      ...
      if (labels.length > 0) {
        for (const [key, a] of Object.entries(labels)) {
          if (
            a.label &&
            a.label[0] &&
            a.label[0].column == "Label" &&
            a.label[0].value
          ) {
            illustrator.draw.draw_label(
              a.tname,
              a.element_id,
              a.label[0].value,
              a.row,
