...
app.get('/', (req, res) => res.send('Log server is listening.'));
...
app.post('/log', formidable(), (req, res) => {
  try {
    ...
    if (activity) {
      ...
      event['cpee:activity_uuid'] = content['activity-uuid'];
    } 
    else event['id:id'] = 'external';
    ...
    switch (topic) {
      ...
      case 'dataelements':
        ...
        event['data'] = { data_changer, data_values }
        break;
      case 'state':
    ...
    const log = YAML.parse(fs.readFileSync('log.yaml', 'utf8')).log;
    ...
    fs.writeFileSync('log.yaml', YAML.stringify({ log }, 2));
  } catch (e) {
    console.error(e);
  } finally {
    res.end();
...
app.post('/data', formidable(), (req, res) => {
  try {
    const notification = JSON.parse(req.fields.notification);
    const { instance, content } = notification;
