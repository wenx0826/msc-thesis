...
function getCurrentNet(checkedState) {
  for (const net of nets) {
    if (net.enabledTrans.length) return net
      ...
      if (checkedState === net.name) continue;
      return net
    }
  }
...
function fire({
  ...
  for (const {
    ...
    if (JSON.stringify(state) === JSON.stringify(newState)) {
      if (!firingTrans.some(ele => ele.transName === transName)) firingTrans.push({
        transName,
        stateName: name
      })
      return;
...
for (const ele of nets[0].firingTrans) {
  ...
  function findNextTrans(stateName) {
    const {
      firingTrans
    } = nets.find(net => net.name === stateName)
